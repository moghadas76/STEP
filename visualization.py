...
def generate_graph():
    ...
    distance = pd.read_csv("/home/seyed/PycharmProjects/step/STEP/datasets/METR-LA/distances_la_2012.csv")
    location = pd.read_csv("/home/seyed/PycharmProjects/step/STEP/datasets/METR-LA/graph_sensor_locations.csv")
    ...
    labels = {
            i: str(tuple(
                location.loc[(location["sensor_id"] == int(rev_mapping[i]))][["latitude", "longitude"]].values.flatten().tolist()
            ))
            for i in net.nodes
    }
    ...
    nx.write_gml(net, "/home/seyed/PycharmProjects/step/STEP/checkpoints/metr-la.gml")
    return net
...
def visualize(net):
    ...
    paths = sorted(Path("/home/seyed/PycharmProjects/step/STEP/loss").iterdir(), key=os.path.getmtime)[:65081]
    loss_ = []

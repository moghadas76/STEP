...
from .mask import MaskGenerator, KNNMaskGenerator, SpatioMaskGenerator
...
class TSFormerSpatialTemporalMasking(TSFormer):


    def __init__(self, patch_size, in_channel, embed_dim, num_heads, mlp_ratio, dropout, num_token, mask_ratio, encoder_depth, decoder_depth, mode="pre-train", spatial_mask_ratio=0.35, node_count=269):
        super().__init__(patch_size, in_channel, embed_dim, num_heads, mlp_ratio, dropout, num_token, mask_ratio, encoder_depth, decoder_depth, mode)
        self.spatial_mask_ratio = spatial_mask_ratio
        self.node_count = node_count
        self.mask = KNNMaskGenerator(num_token, mask_ratio, spatial_mask_ratio, node_count)
        self.output_layer_spatial = nn.Linear(embed_dim, patch_size)
        self.spatial_postional_encoding = SpatialPositionalEncoding(embed_dim, dropout=dropout)
    ...
    def encoding(self, long_term_history, mask=True):
        ...
        # mask
        if mask:
            masked_nodes_spatial, unmasked_nodes_spatial, unmasked_token_index, masked_token_index= self.mask()
            encoder_input = patches[:, unmasked_nodes_spatial, :, :]
            encoder_input = encoder_input[:, :, unmasked_token_index, :]
        else:
            masked_nodes_spatial, unmasked_nodes_spatial, unmasked_token_index, masked_token_index = None, None, None, None
            encoder_input = patches
        # encoding
        hidden_states_unmasked = self.encoder(encoder_input)
        size = num_nodes
        if unmasked_nodes_spatial:
            size = len(unmasked_nodes_spatial)
        hidden_states_unmasked = self.encoder_norm(hidden_states_unmasked).view(batch_size, size, -1, self.embed_dim)

        return hidden_states_unmasked, masked_nodes_spatial, unmasked_nodes_spatial, unmasked_token_index, masked_token_index

    def decoding(self, hidden_states_unmasked, masked_token_index, masked_nodes_spatial, unmasked_token_index):
        """Decoding process of TSFormer: encoder 2 decoder layer, add mask tokens, Transformer layers, predict.
        ...
        Returns:
            torch.Tensor: reconstructed data
        """
        batch_size, _, _, d = hidden_states_unmasked.shape
        
        # encoder 2 decoder layer
        hidden_states_unmasked = self.enc_2_dec_emb(hidden_states_unmasked)  # (8, 269 - MM, 168 - M, 96)

        
        # add mask tokens
        hidden_states_masked_temporal = self.positional_encoding(
            self.mask_token.expand(batch_size, self.node_count, len(masked_token_index), hidden_states_unmasked.shape[-1]),
            index=masked_token_index
            )
        
        hidden_states_masked = self.spatial_postional_encoding(
            self.mask_token.expand(batch_size, len(masked_nodes_spatial), (len(masked_token_index) + hidden_states_unmasked.size(2)), hidden_states_unmasked.shape[-1]),
            index=masked_nodes_spatial
            )
        
        hidden_states_full = torch.cat([hidden_states_unmasked, hidden_states_masked_temporal[:, unmasked_token_index, :, :]], dim=-2)   # B, N, P, d
        hidden_states_full = torch.cat([hidden_states_full, hidden_states_masked], dim=1)
        # decoding
        hidden_states_full = self.decoder(hidden_states_full)
        hidden_states_full = self.decoder_norm(hidden_states_full)

        # prediction (reconstruction)
        reconstruction_full = self.output_layer(hidden_states_full.view(batch_size, hidden_states_unmasked.size(1) + len(masked_nodes_spatial), -1, self.embed_dim))

        return reconstruction_full  #   ([8, 269, 168, 12]
    ...
    def forward(self, history_data: torch.Tensor, future_data: torch.Tensor = None, batch_seen: int = None, epoch: int = None, **kwargs) -> torch.Tensor:
        ...
        if self.mode == "pre-train":
            # encoding
            hidden_states_unmasked, masked_nodes_spatial, unmasked_nodes_spatial, unmasked_token_index, masked_token_index  = self.encoding(history_data)
            # decoding
            reconstruction_full = self.decoding(hidden_states_unmasked, masked_token_index, masked_nodes_spatial, unmasked_token_index=unmasked_nodes_spatial)
            # for subsequent loss computing
            reconstruction_masked_tokens, label_masked_tokens = self.get_reconstructed_masked_tokens(reconstruction_full, history_data, unmasked_token_index, masked_token_index)
            return reconstruction_masked_tokens, label_masked_tokens
        else:
            hidden_states_full, _, _, _, _ = self.encoding(history_data, mask=False)
            return hidden_states_full
        
class TSFormerSpatialMasking(TSFormer):
    ...
    def __init__(self, patch_size, in_channel, embed_dim, num_heads, mlp_ratio, dropout, num_token, mask_ratio, encoder_depth, decoder_depth, mode="pre-train", spatial_mask_ratio=0.35, node_count=269):
        ...
        self.spatial_mask_ratio = spatial_mask_ratio
        self.node_count = node_count
        self.mask = SpatioMaskGenerator(node_count, mask_ratio)
        self.output_layer_spatial = nn.Linear(embed_dim, patch_size)
        self.spatial_postional_encoding = SpatialPositionalEncoding(embed_dim, dropout=dropout)
    ...
    def encoding(self, long_term_history, mask=True):
        ...
        # mask
        if mask:
            masked_nodes_spatial, unmasked_nodes_spatial= self.mask()
            encoder_input = patches[:, unmasked_nodes_spatial, :, :]
        else:
            masked_nodes_spatial, unmasked_nodes_spatial = None, None, None, None
            encoder_input = patches
        # encoding
        hidden_states_unmasked = self.encoder(encoder_input)
        size = num_nodes
        ...
        return hidden_states_unmasked, masked_nodes_spatial, unmasked_nodes_spatial

    def decoding(self, hidden_states_unmasked, masked_nodes_spatial):
        """Decoding process of TSFormer: encoder 2 decoder layer, add mask tokens, Transformer layers, predict.
        ...
        hidden_states_masked = self.spatial_postional_encoding(
            self.mask_token.expand(batch_size, len(masked_nodes_spatial), (hidden_states_unmasked.size(2)), hidden_states_unmasked.shape[-1]),
            index=masked_nodes_spatial
            )
        hidden_states_full = torch.cat([hidden_states_unmasked, hidden_states_masked], dim=1)   # B, N, P, d
        # hidden_states_full = torch.cat([hidden_states_full, hidden_states_masked], dim=1)
        # decoding
        hidden_states_full = self.decoder(hidden_states_full)
        hidden_states_full = self.decoder_norm(hidden_states_full)
    ...
    def forward(self, history_data: torch.Tensor, future_data: torch.Tensor = None, batch_seen: int = None, epoch: int = None, **kwargs) -> torch.Tensor:
        ...
        if self.mode == "pre-train":
            # encoding
            hidden_states_unmasked, masked_nodes_spatial, unmasked_nodes_spatial  = self.encoding(history_data)
            # decoding
            reconstruction_full = self.decoding(hidden_states_unmasked, masked_nodes_spatial)
            # for subsequent loss computing

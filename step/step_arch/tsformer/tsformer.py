...
class TSFormer(nn.Module):
    ...
    def __init__(self, patch_size, in_channel, embed_dim, num_heads, mlp_ratio, dropout, num_token, mask_ratio,
                 encoder_depth, decoder_depth, node_numbers, mode="pre-train", mask_generator_cls=MaskGenerator,
                 masking_strategy="temporal", adj_path="/home/seyed/PycharmProjects/step/STEP/datasets/METR-LA/adj_mx.pkl"):
        super().__init__()
        ...
        # self.graph_encoder = GraphEncoding(embed_dim, dropout=dropout, num_nodes=node_numbers, adj_path=adj_path)
        # # masking
        tks = num_token if masking_strategy == "temporal" else node_numbers
        self.mask_generator_cls = mask_generator_cls
    ...
    def encoding(self, long_term_history, mask=True):
        ...
        """
        batch_size, num_nodes, _, L = long_term_history.shape
        ...
        # patches = self.graph_encoder(patches, device=long_term_history.device)
        # mask
        if mask:
            # graph_obj = None
            # if self.mask_generator_cls == KnnMask:
            ...
            if self.masking_strategy == "temporal":
        ...
        # encoding
        if self.masking_strategy == "temporal":
            # print(encoder_input.shape, "66666666666")
            hidden_states_unmasked, atts = self.encoder(encoder_input)
            hidden_states_unmasked = self.encoder_norm(hidden_states_unmasked).view(batch_size, num_nodes, -1,
                                                                                    self.embed_dim)
        else:
            hidden_states_unmasked, atts = self.encoder(encoder_input)
            #TODO: __int(self.node_numbers * (1 - self.mask_ratio)) + 1____ int(self.node_numbers * (1 - self.mask_ratio))
            _, L_, _, _ =  hidden_states_unmasked.shape
            hidden_states_unmasked = self.encoder_norm(hidden_states_unmasked).view(batch_size,
                                                                                    L_,
                                                                                    -1,
                                                                                    self.embed_dim)
        return hidden_states_unmasked, unmasked_token_index, masked_token_index, atts
    ...
    def decoding(self, hidden_states_unmasked, masked_token_index):
        ...
        """
        batch_size, num_nodes, L, _ = hidden_states_unmasked.shape
        ...
        if self.masking_strategy == "temporal":
            ...
            # hidden_states_masked = self.graph_encoder(hidden_states_masked, device=hidden_states_masked.device)
            ...
            hidden_states_masked = self.positional_encoding(
                self.mask_token.expand(batch_size, L, len(masked_token_index),
                                       hidden_states_unmasked.shape[-1]),
                index=masked_token_index
            ).transpose(-2, -3)
            hidden_states_full = torch.cat([hidden_states_unmasked, hidden_states_masked], dim=-3)  # B, N, P, d
            ...
            # prediction (reconstruction)
            reconstruction_full = self.output_layer(hidden_states_full.view(batch_size, L, -1, self.embed_dim))
    ...
    def get_reconstructed_masked_tokens(self, reconstruction_full, real_value_full, unmasked_token_index,
        ...
        batch_size, num_nodes, L, _ = reconstruction_full.shape
        if self.masking_strategy == "temporal":
            ...
            batch_size, P, _,  L = reconstruction_full.shape
            reconstruction_masked_tokens = reconstruction_full[:, :, len(unmasked_token_index):, :]  # B, N, r*P, d
            reconstruction_masked_tokens = reconstruction_masked_tokens.reshape(batch_size, L * P, -1)
        ...
        if self.masking_strategy == "temporal":
            ...
            try:
                #TODO:
                # - int(self.node_numbers * self.mask_ratio) ?
                _, _L, _, _ = label_masked_tokens.shape
                label_masked_tokens = label_masked_tokens.view(batch_size, _L,
                                                               -1).transpose(1, 2)
    ...
    def forward(self, history_data: torch.Tensor, future_data: torch.Tensor = None, batch_seen: int = None,
                epoch: int = None, **kwargs) -> Tuple[torch.Tensor, ...]:
        ...
        if self.mode == "pre-train":
            ...
            hidden_states_unmasked, unmasked_token_index, masked_token_index, atts = self.encoding(history_data)
            # decoding
            ...
            hidden_states_full, _, _, atts = self.encoding(history_data, mask=False)
            return hidden_states_full, atts
